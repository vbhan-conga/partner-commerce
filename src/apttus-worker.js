(function () {
    ...
    class AppVersion {
        constructor(scope, adapter, database, idle, manifest, manifestHash) {
            ...
            const locPrefix = self.location.pathname.substring(0, self.location.pathname.indexOf('/apttus-worker.js'));
            Object.keys(this.manifest.hashTable).forEach(url => {
                this.hashTable.set(locPrefix + url, this.manifest.hashTable[url]);
            });
        ...
        initialize() {
            return __awaiter$5(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    const locPrefix = self.location.pathname.substring(0, self.location.pathname.indexOf('/apttus-worker.js'));
                    if(manifest && manifest.assetGroups && manifest.assetGroups.length){
                        manifest.assetGroups.forEach(assetGroup => {
                            assetGroup.urls = assetGroup.urls.map(url => locPrefix + url);
                        });
                    }
